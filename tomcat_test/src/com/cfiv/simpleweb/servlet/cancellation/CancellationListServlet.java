...
import com.cfiv.simpleweb.task.ContractDataListTask;
 ...
 * @author tanimura
 */
@WebServlet(name = "CancellationListServlet", urlPatterns = { "/CancellationListServlet" })
public class CancellationListServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        // 登録種別
        int entryKind = Defines.ENTRYKIND_CANCELLATION;
        ...
        try {
            ...
            if (customerID != null) {
                ...
                // 契約情報取得タスクを生成する
                ContractDataListTask contractTask = new ContractDataListTask(customerID);
                ...
                session.setAttribute(Defines.UPDATE_DATE, contractTask.getUpdateDateString());
                session.setAttribute(Defines.CONTRACT_DATALIST, contractTask.getContractDataList());
                ...
                Log.log(Log.INFO, logHeader + "契約情報更新日(前月末日) = " + contractTask.getUpdateDateString());
                Log.log(Log.INFO, logHeader + "契約情報件数 = " + contractTask.getContractDataList().size());
